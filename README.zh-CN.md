...
Codex Web 是一个非官方、自托管的 OpenAI Codex CLI 网页工作台。它提供持久化会话、未发送草稿、附件与交付文件、服务器端任务排队、实时引导、一次性定时或外部事件续跑、可续接的终止/中断记录、会话归档、完整工作记录、完成任务未读提示、引用提问、自动命名、字号调节以及可选的语音转写。
...
已完成的会话可以无损归档和恢复；当 Codex rollout 达到 500 MiB 时，界面会提示新建任务以控制超长上下文成本。会话容量菜单同时显示 rollout 大小、最近输入上下文/模型窗口和当前 Codex 套餐剩余额度。Markdown 文件可以打开原文或进入登录态保护的阅读模式，其中 LaTeX 公式按需渲染；HTML 在无脚本、无同源权限的沙箱中预览。文本下载响应明确声明 UTF-8，保持 iOS Safari 中文预览稳定。

移动 Safari 使用固定应用外壳，只让内容区滚动。语音转写会对短暂连接和上游失败进行有界重试。本地 Excel 附件由托管的 openpyxl/pandas 技能处理，详细 Excel 规则只在本轮确实包含对应附件时注入。Apps、连接器、Goals 和多代理能力默认关闭，仅在用户明确提出时启用。
