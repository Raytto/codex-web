...
运行中的工作记录默认打开到最新状态，并会持续跟随更新，直到你主动向上滚动。用户终止任务后，关键执行过程会保留为一条历史消息，下一轮可从终止处继续。本地 Excel 附件由托管的 openpyxl/pandas 技能处理，详细 Excel 规则只在本轮确实包含对应附件时注入。Apps、连接器、Goals 和多代理能力默认关闭，仅在用户明确提出时启用。
