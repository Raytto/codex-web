...
- The default profile contains no host-root bridge, Docker socket, or host filesystem mount. Optional bridge and Remote Worker modules are present for operators who make an explicit trust decision and supply their own paths and credentials. A host-root bridge is a separate root process, not a harmless container toggle; obtain explicit consent, limit its absolute paths, and verify the socket ACL before starting it.
