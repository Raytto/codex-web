...
export function buildSubagentActivity(activities: JobEvent[]): SubagentActivitySummary {
  ...
  const ordered = [...agents.values()].sort((left, right) => {
    const activeDifference = Number(ACTIVE_STATUSES.has(right.status)) - Number(ACTIVE_STATUSES.has(left.status));
    if (activeDifference) return activeDifference;
    return timestamp(left.startedAt) - timestamp(right.startedAt) || left.name.localeCompare(right.name);
  });
  ...
  return {
    ...
    completedCount: done.filter((agent) => agent.status === "completed").length,
    failedCount: done.filter((agent) => agent.status === "failed" || agent.status === "interrupted").length,
  };
}
...
function mergeAgentState(agents: Map<string, SubagentView>, state: SubagentEventState, createdAt: string | null): void {
  ...
  agents.set(id, {
    id,
    path,
    name: agentName(path, id),
    status: nextStatus,
    summary,
    startedAt: existing?.startedAt ?? createdAt,
    updatedAt: createdAt ?? existing?.updatedAt ?? null,
  });
}

function normalizeStatus(value: unknown): SubagentStatus | null {
  return value === "pending" || value === "running" || value === "completed" || value === "failed" || value === "interrupted"
    ? value
    : null;
}

function agentName(path: string, id: string): string {
